...
public class BlockJsonModelBake {
    ...
    private static void assignUVForFace(ModelJson.Face f, EnumFacing face, int[] ids, float[] outU, float[] outV,
        ...
        // Scale UV coordinates for non-standard texture sizes
        // UV values in JSON are in texture pixel space; IIcon expects 0-16 range
        if (textureSize != null && textureSize.length >= 2
                && (textureSize[0] != 16 || textureSize[1] != 16)) {
            float scaleU = 16.0f / textureSize[0];
            float scaleV = 16.0f / textureSize[1];
            for (int i = 0; i < 4; i++) {
                outU[i] *= scaleU;
                outV[i] *= scaleV;
            }
