 ...
 *       CatFrame 同样由管线层统一处理（见
 *       {@link decok.dfcdvadstf.catframe.model.render.pipeline.GlintPass}），本类不再参与</li>
...
public class GuiGraphicsExtractor {
     ...
     *   <li>委托 {@link IItemStateProvider#render} 渲染模型（附魔光效由管线
     *       {@code GlintPass} 统一叠加）</li>
     * </ol>
     */
    ...
    private void drawItemModel(ItemStack stack, @Nullable float[] pose, boolean allowOversized) {
        ...
        try {
            ...
            // 附魔光效由管线 FeatureRenderDispatcher → GlintPass 统一叠加，本类不再处理
     ...
     * 深度测试保持启用，确保模型写入深度缓冲供管线 {@code GlintPass} 的
